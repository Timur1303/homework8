package kg.geektech.homework8.general;

import kg.geektech.homework8.players.*;
...
public class RPG_Game {
    ...
    private static void printStatistics(Boss boss, Heroes[] heroes, String round_name) {
    ...
    private static void heroesHit(Boss boss, Heroes[] heroes) {
        for (int i = 0; i < heroes.length; i++) {
    ...
    private static void heroesApplySuperAbilities(Boss boss, Heroes[] heroes) {
        for (int i = 0; i < heroes.length; i++) {
            if (heroes[i].getHealth() > 0) {
    ...
    private static boolean isFinished(Boss boss, Heroes[] heroes) {
        if (boss.getHealth() <= 0) {
